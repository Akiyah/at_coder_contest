...
max_length = 0
heights = []
depth_leafs.keys.sort.reverse.each do |d|
  pp [d, depth_leafs[d], heights] if $debug
  depth_leafs[d].each do |l|
    h = 0
    while l
      if heights[l]
        max_length = [max_length, heights[l] + h + 1].max
        l = nil
      else
        heights[l] = h
        max_length = [max_length, h + 1].max
        h += 1
        l = up_paths[l]
      end
    end
